...
import com.xavier.starter.canal.client.exception.CanalClientException;
...
public abstract class AbstractBasicMessageTransponder extends AbstractMessageTransponder {
	...
	protected void distributeEvent(Message message) {
		...
		for (CanalEntry.Entry entry : entries) {
			...
			for (CanalEntry.RowData rowData : rowChange.getRowDatasList()) {
				...
				distributeByImpl(entry.getHeader().getTableName(), rowChange.getEventType(), rowData);
	 ...
	 */
	protected void distributeByImpl(String tableName, CanalEntry.EventType eventType, CanalEntry.RowData rowData) {
		if (listeners != null) {
			for (CanalEventListener listener : listeners) {
				listener.onEvent(tableName, eventType, rowData);
			}
		}
	}
	...
	protected void distributeByAnnotation(String destination,
		...
		annoListeners.forEach(point -> point
				...
				.forEach(entry -> {
					...
					try {
						Object[] args = getInvokeArgs(method, tableName, eventType, rowData);
	 ...
	 */
	protected abstract Object[] getInvokeArgs(Method method, String tableName, CanalEntry.EventType eventType,
	                                          CanalEntry.RowData rowData);
